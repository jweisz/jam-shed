...
class VirtualRhythmGuitarist(VirtualInstrumentalist):
    ...
    def tick(self, state: Dict[str, Any], beat: int, sub_beat: int) -> None:
        ...
        if beat == 0 and sub_beat == 0 and current_bar % 4 == 0:
            ...
            if self.style == PlayingStyle.FUNK:
                style_patterns = ["funk_shuffle", "syncopated_rock"]
            elif self.style == PlayingStyle.BLUES:
                style_patterns = ["blues_shuffle", "steady_8th"]
            elif state.get("intensity", 80) > 100:
                style_patterns = ["driving_rock", "syncopated_rock"]
    ...
    def play_note(self, state: Dict[str, Any], beat: int, sub_beat: int, ghost: bool = False) -> None:
        ...
        for i, note in enumerate(chord_notes[:4]):
            ...
            def trigger(n=note, v=velocity):
                note_off_ref: list[threading.Timer] = []

                def note_off_callback() -> None:
                    self._note_off_with_cleanup(n, note_off_ref[0])

                note_off_timer = threading.Timer(0.2, note_off_callback)
                note_off_ref.append(note_off_timer)
                self._active_timers.append(note_off_timer)
                self.midi.send_message([0x90 | self.channel, n, v])
                self._playing_notes.add(n)
                note_off_timer.start()

            if strum_delay > 0:
                t = threading.Timer(strum_delay, trigger)
        ...
        if self.on_play_callback:
            self.on_play_callback(self.name, chord_notes[0])
